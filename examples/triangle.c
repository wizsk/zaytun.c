...
// This idea is that you can take this code and compile it to different
// platforms with different rendering machanisms: native with SDL, WebAssembly
// with HTML5 canvas, etc.
#include "zaytun.c"

#define WIDTH 800
#define HEIGHT 600
#define CIRCLE_RADIOUS 100
#define CIRCLE_COLOR 0x88FF0000
...
uint32_t *render(float dt) {
...

  {
    ...
    rotate_point(&x2, &y2);
    rotate_point(&x3, &y3);
    fill_triangle(pixels, WIDTH, HEIGHT, x1, y1, x2, y2, x3, y3, 0xFF2020AA);
  }

  {
    float x = circle_x + circle_dx * dt;
    if (x - CIRCLE_RADIOUS < 0 || x + CIRCLE_RADIOUS >= WIDTH) {
      circle_dx *= -1;
    } else {
      circle_x = x;
    }
    ...
    if (y - CIRCLE_RADIOUS < 0 || y + CIRCLE_RADIOUS >= HEIGHT) {
      circle_dy *= -1;
    } else {
      circle_y = y;
    }

    fill_circle(pixels, WIDTH, HEIGHT, circle_x, circle_y, CIRCLE_RADIOUS, CIRCLE_COLOR);
  }
...

  {
    ...
    for (;;) {
      ...
      for (size_t y = 0; y < HEIGHT; ++y) {
        memcpy(pixels_dst + y * pitch, pixels_src + y * WIDTH,
